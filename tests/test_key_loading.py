...
from datum.api.internals.auth import PUBLIC_KEY_PATH_VARIABLE
...
def test_the_key_is_read_from_the_file_the_variable_names(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(PUBLIC_KEY_PATH_VARIABLE, str(key))
...
def test_a_missing_file_is_a_startup_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(PUBLIC_KEY_PATH_VARIABLE, str(tmp_path / "absent.pub"))
